...
sequential_impls = ["sequential", "sorted_buildup_sequential"]
...
for dataset in datasets:
    ...
    for impl in impls:
        ...
        means = means[means["test"] == "t_insert"]
        means = means[means["key"] == "total"]

        sns.lineplot(data=means, x="words", y="value", ax=axs[0], label=impl)
        axs[0].set_title(f"{dataset}: Trie Buildup, {threads[-1]} threads: Buildup time, {iterations} iterations")
        ...
        axs[1].set_title(f"{dataset}: Trie Buildup, {datasets[dataset]['word_counts'][-1]} words: Buildup time, {iterations} iterations")
        ...
        mean_sequential = means[means["impl"] == "sequential"]["value"].mean()
        ...
        axs[2].set_title(f"{dataset}: Trie Buildup, {datasets[dataset]['word_counts'][-1]} words: Speedup, {iterations} iterations")
        ...
        axs[3].set_title(f"{dataset}: Trie Buildup, {datasets[dataset]['word_counts'][-1]} words: Efficiency, {iterations} iterations")
        ...
        fig.tight_layout()
        fig.savefig(f"trie_buildup_{dataset}.pdf")
